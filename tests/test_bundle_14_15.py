...
from cogniverse_framework.claims import EvidenceLinker, LearningClaim
...
class TestBundle1415(unittest.TestCase):
    ...
    def test_learning_claim(self):
        claim = LearningClaim("prefer_branches", 0.8, ["a"])
        self.assertTrue(claim.validate()["valid"])

    def test_evidence_linker(self):
        result = EvidenceLinker().build("strategy", ["a", "b"], ["c"])
        self.assertEqual(result["confidence"], 0.667)

    def test_adapter_contract(self):
        result = Exp042Adapter(
            learning_evidence={"strategy": "injected_from_lab"},
        ).run()

        self.assertIn("learning_evidence", result)
        self.assertEqual(
